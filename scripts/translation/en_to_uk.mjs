...
jsonFiles.forEach(fileName => {
    const inputFullPath = resolve(projectRoot, inputDirRelativePath, fileName);
  ...
  try {
    ...
    const ukJson = translateObjectReverse(enJson);
      ...
      const baseFilename = basename(inputFullPath);
    let outputFilename;
      let subDir = '';
    ...
    if (baseFilename.startsWith('plan_')) {
      ...
      const outputFullPath = join(outputDir, subDir, outputFilename);
